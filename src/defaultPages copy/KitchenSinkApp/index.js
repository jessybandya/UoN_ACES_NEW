...
class KitchenSinkApp extends React.PureComponent {
  ...
  login = (uid) => {
    ...
    this.uid = auth1?.currentUser?.uid;
    this.props.onLogin(this.uid, COMETCHAT_CONSTANTS.AUTH_KEY);

    console.log("CurrentUserId: ",auth1?.currentUser?.uid)
  }
...
<br/>
          <div css={uidWrapperStyle()}>
            ...
            <div css={loginBtn()}><button type="button" onClick={() => this.login(`${auth1?.currentUser?.uid}`)}>Proceed</button></div>
